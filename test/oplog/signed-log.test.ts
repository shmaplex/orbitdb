...
describe("Signed Log", () => {
  ...
  it("has the correct public key", async () => {
    ...
    expect(log.identity.publicKey).toBe(testIdentity1.publicKey);
  });

  it("has the correct pkSignature", async () => {
    const log = await Log(testIdentity1, { logId: "A" });
    expect(log.identity.signatures.id).toBe(testIdentity1.signatures.id);
  });

  it("has the correct signature", async () => {
    const log = await Log(testIdentity1, { logId: "A" });
    expect(log.identity.signatures.publicKey).toBe(
      testIdentity1.signatures.publicKey
    );
  });
  ...
  it("throws an error if log is signed but trying to merge with an entry that doesn't have public signing key", async () => {
    ...
    expect(err).toBe("Error: Entry doesn't have a key");
  });
  ...
  it("throws an error if log is signed but trying to merge an entry that doesn't have a signature", async () => {
    ...
    expect(err).toBe("Error: Entry doesn't have a signature");
  });
  ...
  it("throws an error if log is signed but the signature doesn't verify", async () => {
    ...
    expect(err).toBe(
      `Error: Could not validate signature for entry "${entry.hash}"`
    );
    expect(values.length).toBe(1);
    expect(values[0].payload).toBe("one");
  });
