...
describe("ConflictResolution", () => {
  describe("NoZeroes", () => {
    it("passed function cannot return 0", () => {
      ...
      const func = (_a: any, _b: any) => 0;
      const sortFn = ConflictResolution.NoZeroes(func);
      const expected =
        "Error: Your log's tiebreaker function, func, has returned zero and therefore cannot be";
  ...
  describe("SortByClockId", () => {
    let fallbackFn: (a: any, b: any) => any;

    beforeAll(() => {
      fallbackFn = (a: any, _b: any) => a;
    });
    ...
    it("returns -1 when first clock's id is less than second clock's", () => {
      ...
      strictEqual(
        ConflictResolution.SortByClockId(record1, record2, fallbackFn),
        -1
      );
    });
  ...
  describe("SortByClocks", () => {
    ...
    it("returns -1 when a's time is less than b's", () => {
      ...
      strictEqual(
        ConflictResolution.SortByClocks(record1, record2, fallbackFn),
        -1
      );
    });
  ...
  describe("Last write wins", () => {
    it("returns -1 when a's time is less than b's", () => {
      const record1 = { clock: Clock("A", 1) };
      const record2 = { clock: Clock("B", 2) };
      strictEqual(ConflictResolution.LastWriteWins(record1, record2), -1);
    });
    ...
    it("returns the clock when a and b are the same", () => {
      ...
      const expected = { clock: Clock("A") };
      deepStrictEqual(
        ConflictResolution.LastWriteWins(record1, record2),
        expected
      );
    });
  });
