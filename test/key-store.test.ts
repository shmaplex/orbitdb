...
describe("KeyStore", () => {
  ...
  describe("Creating and retrieving keys", () => {
    ...
    it("throws an error when creating a key without an id", async () => {
      await expect(keystore.createKey()).rejects.toThrow(
        "id needed to create a key"
      );
    });

    it("throws an error when creating a key with a null id", async () => {
      await expect(keystore.createKey(null)).rejects.toThrow(
        "id needed to create a key"
      );
    });

    it("returns true if key exists", async () => {
      const hasKey = await keystore.hasKey("key1");
      expect(hasKey).toBe(true);
    });

    it("returns false if key does not exist", async () => {
      const hasKey = await keystore.hasKey("key1234567890");
      expect(hasKey).toBe(false);
    });

    it("throws an error when checking if key exists when no id is specified", async () => {
      await expect(keystore.hasKey()).rejects.toThrow(
        "id needed to check a key"
      );
    });

    it("gets a key", async () => {
      const keys = await keystore.createKey("key1");
      const result = await keystore.getKey("key1");
      expect(result).toEqual(keys);
    });

    it("throws an error when getting a key without an id", async () => {
      await keystore.createKey("key1");
      await expect(keystore.getKey()).rejects.toThrow("id needed to get a key");
    });

    it("throws an error when getting a key with a null id", async () => {
      await keystore.createKey("key1");
      await expect(keystore.getKey(null)).rejects.toThrow(
        "id needed to get a key"
      );
    });

    it("gets a non-existent key", async () => {
      const actual = await keystore.getKey("key111111111");
      expect(actual).toBeUndefined();
    });

    it("doesn't create a key when keystore is closed", async () => {
      await keystore.close();
      await expect(keystore.createKey(id)).rejects.toThrow(
  ...
  describe("Options", () => {
    ...
    describe("Using default options", () => {
      ...
      it("uses default storage and default path to retrieve a key", async () => {
        const key = await keystore.getKey("key1");
        expect(key).toEqual(unmarshalledPrivateKey);
      });
    });
    ...
    describe("Setting options.storage", () => {
      ...
      it("uses the given storage to retrieve a key", async () => {
        const key = await keystore.getKey("key2");
        expect(key).toEqual(unmarshalledPrivateKey);
      });
    });
    ...
    describe("Setting options.path", () => {
      ...
      it("uses default storage using given path to retrieve a key", async () => {
        const key = await keystore.getKey("key3");
        expect(key).toEqual(unmarshalledPrivateKey);
      });
    });
  });
  ...
  describe("Using keys for signing and verifying", () => {
    ...
    describe("Getting the public key", () => {
      ...
      it("gets the public key buffer", async () => {
        ...
        expect(uint8ArrayToString(publicKey, "base16")).toBe(expected);
      });

      it("throws an error if no keys are passed", async () => {
        await expect(keystore.getPublic()).rejects.toThrow();
      });

      it("throws an error if a bad format is passed", async () => {
        await expect(
          keystore.getPublic(key, { format: "foo" } as any)
        ).rejects.toThrow();
      });
    });

    describe("Verifying", () => {
      let key: any, publicKey: string;
      ...
      it("verifies content with cache", async () => {
        ...
        expect(first - startTime).toBeGreaterThan(after - first);
      });
