...
describe("Storages", () => {
  ...
  // Utility: run storage test
  const runTestWithStorage = async (storage: any) => {
    const amount = 100;
    ...
    const result: any[] = [];
    for await (const v of log1.storage.iterator()) result.push(v);
    for await (const v of log2.storage.iterator()) result.push(v);
  ...
  // Utility: check storage interface
  const testInterface = (storage: any) => {
