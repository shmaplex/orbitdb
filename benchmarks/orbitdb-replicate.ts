import { createOrbitDB, EventsInstance } from "../src/index.js";
...
(async () => {
  ...
  const db1 = (await orbitdb1.open("benchmark-replication", {
    type: "events",
  })) as EventsInstance;
  ...
  console.log(
    `Adding ${entryCount} events took ${duration1} ms, ${operationsPerSecond1} ops/s, ${millisecondsPerOp1} ms/op`
  );
  ...
  const onJoin = async (_peerId: string) => {
    connected = true;
  };
  ...
  console.log(
    `Replicating ${entryCount} events took ${duration2} ms, ${operationsPerSecond2} ops/s, ${millisecondsPerOp2} ms/op`
  );
