...
/**
 * Loops indefinitely, appending to the log and updating metrics.
 */
...
(async () => {
  ...
  const interval = setInterval(async () => {
    ...
    if (seconds % 10 === 0) {
      console.log(
        `--> Average of ${lastTenSeconds / 10} q/s in the last 10 seconds`
      );
    ...
    console.log(
      `${queriesPerSecond} queries per second, ${totalQueries} queries in ${seconds} seconds`
    );
    queriesPerSecond = 0;
  }, 1000);
